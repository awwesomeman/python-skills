...
if [ -z "$EXTRACTED_DIR" ] || [ ! -f "$EXTRACTED_DIR/uninstall.sh" ]; then
  echo -e "${YELLOW}[ERROR] uninstall.sh not found in downloaded archive.${NC}"
  exit 1
fi

echo -e "${BLUE}Running uninstaller...${NC}"
echo ""
bash "$EXTRACTED_DIR/uninstall.sh" "$@"

echo -e "${GREEN}Remote uninstall complete. Temporary files cleaned up.${NC}"
