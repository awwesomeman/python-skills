...
### 過擬合防護
IS 表現優於 OOS 是常態而非例外。參數越多、回測期越短，衰減越嚴重。經驗參考：自由參數數量宜 < 樣本年數的 2 倍（此為寬鬆上界；低頻策略或樣本期 < 5 年時建議 < 樣本年數；亦可搭配 AIC/BIC 等資訊準則做量化判斷）。策略邏輯應有明確的經濟學直覺支撐，純統計挖掘的策略衰減最快。
...
- **IC 加權**：依歷史 IC 均值分配權重。🚨 IC 必須用 rolling 歷史值（非全段），否則前視偏誤。IC < 0 且有經濟直覺支持反向者，應**翻轉信號方向**（乘以 -1）後重新計算 IC；IC < 0 且無法解釋者，權重設為 0
- **最大化 IR**：理論最優但對估計誤差極敏感，實盤中等權往往更穩健

### 因子相關性
🚨 高度相關的因子（如不同 lookback 的動能）在等權組合中隱性 over-weight 同一主題。建議 corr > 0.5~0.7 的群組（視因子數量與去冗餘嚴格度調整）只保留 IC 最高的一個，或先 PCA 降維。corr 0.5 時 R² 已達 25%，邊際分散效果有限。
...
### Mean-Variance 最佳化
🚨 三大陷阱：
1. 樣本共變異數矩陣在 N > T 時極不穩定 — **必須用 shrinkage 或結構化估計器**（如 Ledoit-Wolf、Factor Model），不可直接使用樣本共變異數
2. 對預期收益 μ 極度敏感 — 微小變動 → 權重劇烈翻轉。實盤建議 Black-Litterman 或直接 minimum variance（不依賴 μ）
3. MVO 為凸二次規劃，理論上無局部解問題，但資產數 > 200 時共變異數矩陣更易**病態 (ill-conditioned)**，solver 效能下降且解的數值穩定性堪憂
