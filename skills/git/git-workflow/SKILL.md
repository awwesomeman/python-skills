...
> **避免 emoji**：issue 無 parser 鏈，emoji 不致破壞自動化（與 commit 強制禁用的規則強度不同），但仍會讓 `gh issue list` / 通知 email 視覺不一致；軟性建議。
>
> **語言一致**：同一 repo 內 issue / PR / commit body 採單一語言（中/英文擇一）。混用會讓 `gh issue list`、GitHub 搜尋、`git log` 視覺斷裂、全文搜索分散。不規定語言，但選定後須一致。
