...
| **執行 status** | 「在哪個分支」「等 review」「已 merge」 | Labels、Linked PR、`Closes #N`（不寫進 body） |

> **Why 過程討論留在 comments**：Plan 在開發中經常微調，寫進 body 會跟最終實作不一致；comments 有時序，自然反映演進，body 則須隨時可信。
>
> **定案回灌**：若 plan 是專案長期參考的設計決策（非單次實作步驟），定案後在 body 加 `## Proposed Solution` 區段——後續接手者只看 body 即可掌握「現狀 + 解法」。單次實作步驟則留在 comments，不回灌。
