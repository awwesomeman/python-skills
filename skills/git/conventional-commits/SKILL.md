...
| **版本號命名一致性** | 確保 Git Tag 與 Changelog 遵循專案約定（通常 Tag 帶 `v`，Changelog 不帶） |
...
## 版本發布與自動化管理 (Release & Automation)

> 建議使用 [commitizen](https://commitizen-tools.github.io/commitizen/) 進行語意化版本管理。請確保 `pyproject.toml` 已正確設定 `[tool.commitizen]`。

### 版本命名慣例 (Version Naming Convention)

為了平衡機器可讀性與人類閱讀體驗，請遵循以下慣例：

| 位置 | 格式範例 | 說明 |
|------|----------|------|
| **Git Tag** | `v1.2.3` | 帶 `v` 前綴。方便在 Git refs 中識別版本標籤。 |
| **CHANGELOG.md** | `## [1.2.3]` | **不帶 `v` 前綴**。符合 SemVer 純數字定義，視覺更簡潔。 |
| **Commit Message** | `chore(release): v1.2.3` | 帶 `v` 前綴。作為版本發布的明確標記。 |

### 標準發布流程 (Standard Release SOP)

執行 Release 時，請依序執行以下步驟，切勿跳過：

1. **環境準備**：切換至 `main` 分支並拉取最新代碼。
   ```bash
   git checkout main && git pull
   ```
2. **品質驗證**：確保所有測試通過且 Lint 無誤。
   ```bash
   uv run pytest    # 或專案對應的測試指令
   ```
3. **自動改版 (Bump)**：執行 `cz bump`。
   * 工具會根據 Commit History 計算新版號。
   * 自動更新 `pyproject.toml`、`__init__.py`。
   * **自動生成 CHANGELOG.md**（遵循無 `v` 前綴規範）。
   * 自動建立一個 release commit 與對應的 Git Tag。
   ```bash
   cz bump --dry-run  # 建議先預覽
   cz bump
   ```
4. **推送到遠端**：務必包含 tags。
   ```bash
   ...
   ```
5. **同步下游 (選配)**：若專案被其他 Repo 引用（如 submodule），需前往該 Repo 更新指針。
...
> **注意：Pre-1.0 版本 (0.y.z)**
> 根據 SemVer 規範，在 `1.0.0` 之前，API 尚未穩定。此時 **MINOR (0.y.z)** 的變動通常就被視為 Breaking Change。建議在專案初期透過 `major_version_zero = true` 設定來管理。

### 常用指令備忘

*   **`cz commit` (或 `cz c`)**：互動式提交。會以問答形式引導填寫，自動確保格式正確，並支援 Breaking Change 標記。
*   **`cz bump`**：全自動流程。分析 commit、計算版本、更新檔案、生成日誌並打標籤。
*   **`cz changelog`**：手動操作。當需要單獨更新或修復 `CHANGELOG.md` 時使用。
...
- [ ] **若準備執行 Release**：
    - [ ] 已確認 `cz bump --dry-run` 的版本升級邏輯正確？
    - [ ] 已確認 `CHANGELOG.md` 生成內容符合預期？
    - [ ] 是否已準備好 `git push --tags`？
